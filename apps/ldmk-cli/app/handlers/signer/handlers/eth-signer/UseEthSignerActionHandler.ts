...
export class UseEthSignerActionHandler implements SignerActionHandler {
  ...
  private async executeAction(
    ...
    for (const ethSignerActionHandler of this.ethSignerActionHandlers) {
      if (ethSignerActionHandler.supports(actionType)) {
        return await ethSignerActionHandler.handle();
      }
    }
    return false;
  }
